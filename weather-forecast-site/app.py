            ...
            [
                ...
                html.Div(
                    [
                        ...
                        # график т-ры
                        dcc.Graph(
                            id="temperature-graph", style={"height": "300px"}
                        ),
                        # осадков
                        dcc.Graph(
                            id="precipitation-graph", style={"height": "300px"}
                        ),
                    ]
                ),
            ]
...
def update_graphs(stored_data, selected_days):
    ...
    colors = px.colors.qualitative.Set3  # цветовая палитра
    for i, point in enumerate(stored_data):
        dates = list(map(
            lambda x: x["Date"],
            point["DailyForecasts"][:selected_days]
        ))
        temps = list(map(
        ...
        precips = list(map(
            lambda x: x["Day"]["PrecipitationProbability"],
            point["DailyForecasts"][:selected_days]
        ))
